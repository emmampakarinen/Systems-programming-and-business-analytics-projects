...
void uncomp_file_stdout(char *input) {
    ...
    while (fread(&character.n, sizeof(character.lkm), 1, file) > 0) { // uncompressing the file
        fread(&character.c, sizeof(character.c), 1, file); 
        for (int i = 0; i < character.lkm; i++) { // printing the same sequential characters
