// Emma Pakarinen, 15.1.2023

/* object: my-grep, a variant of the UNIX tool grep. This tool looks through a file, line by line, trying to find a user-specified 
search term in the line. If a line has the word within it, the line is printed out, otherwise it is not. */
...
void grep_file(char *keyword, char *input) {
    ...
    while (row_size >= 0) { // looping as long as eof is foung
        char *check;
        check = strstr(buffer, keyword); // from: https://cplusplus.com/reference/cstring/strstr/
